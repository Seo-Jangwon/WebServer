...
void handle_static_file(SOCKET client_socket, const http_request *req, const char *request_path) {
    ...
    if (!is_path_safe(request_path)) {
        error_context err = MAKE_ERROR_DETAIL(ERR_ACCESS_DENIED,
                                              "Access Denied",
                                              request_path);
        log_error(&err);
        send_error_response(client_socket, &err);
    ...
    if (file.status_code != 200) {
        ...
        if (file.status_code == 404) {
            err = MAKE_ERROR_DETAIL(ERR_NOT_FOUND,
                                    "The requested file was not found",
                                    file_path);
        } else if (file.status_code == 403) {
            err = MAKE_ERROR_DETAIL(ERR_ACCESS_DENIED, "Access Denied", file_path);
        } else {
            err = MAKE_ERROR_DETAIL(ERR_INTERNAL_ERROR,
                                    "Failed to read file",
    ...
    while (remaining > 0) {
        ...
        while (sent < chunk_size) {
            ...
            if (result == SOCKET_ERROR) {
                ...
                error_context err = MAKE_ERROR_DETAIL(ERR_SOCKET_ERROR,
